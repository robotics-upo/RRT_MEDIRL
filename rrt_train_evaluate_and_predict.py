...
if __name__ == '__main__':
  ...
  input_fn = tf.estimator.inputs.numpy_input_fn( x={'images': data_node.data[:,:,:,0],'images_obj': data_node.data_obj[:,:,:,0], 'labels': data_node.labels[:,:,:,0]}, y = data_node.labels[:,:,:,0], batch_size=batch_size, num_epochs=None, shuffle=True)
  ...
  evaluate_input_fn = tf.estimator.inputs.numpy_input_fn( x={'images': test_node.data[:,:,:,0],'images_obj': test_node.data_obj[:,:,:,0], 'labels': np.zeros_like(test_node.data[:,:,:,0])}, num_epochs=1, shuffle=False)
  ...
  metrics = model.evaluate(input_fn=evaluate_input_fn)
